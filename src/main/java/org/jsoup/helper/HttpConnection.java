...
import cn.ryan.io.cache.mode.InputStreamCacher;
...
public class HttpConnection implements Connection {
	...
	private static final int HTTP_TEMP_REDIR = 307; // http/1.1 temporary
													...
													// set.
	private final static  String DEFAULT_FILE_PATH = System.getProperty("user.dir") + "/temp/";
	...
	public static Connection setConfigParameter(Processor p){
		Connection con = new HttpConnection();
		CrawlerSite cs = p.getSite();
		con.timeout(cs.getTimeOut())
		.userAgent(cs.getUserAgent())
		.ignoreContentType(cs.isIgnoreContentType())
		.method(cs.getMethod())
		.headers(cs.getHeaders());
		return con;
	}
	...
	public Connection filePath(String filePath){
		if (filePath == null) {
		} else {
			if (!filePath.endsWith("/")) {
				filePath += "/";
			}
			if (!new File(filePath).exists()) {//判断用户输入的目录是否存在
				Validate.isNotExists("Fill in the filePath path does not exist");
			}
			File file = new File(filePath);//判断用户输入的目录是否存在
			if (!file.exists()) {
				file.mkdir();//不存在则新建
			}
		}
	...
	private static abstract class Base<T extends Connection.Base> implements Connection.Base<T> {
		...
		public T url(URL url) {
			...
			if (url.toString().matches("(https|HTTPS|Https)://(.*?)")) {
				trustAllHosts();
			}
			this.url = url;
			return (T) this;
		...
		public String header(String name) {
			...
			if (val != null) {
				// headers should be ISO8859 - but values are often actually
				// UTF-8. Test if it looks like UTF8 and convert if so
	...
	public static class Response extends HttpConnection.Base<Connection.Response> implements Connection.Response {
		...
		public static void inputStreamToFile(Connection.Request req, InputStreamCacher inc) {
			if (null == req.filePath()) {
				return;
			}
			FileOutputStream os = null;
			try {
				InputStream in = inc.getInputStream();
				if (in == null) {
					return;
				}
				File file = new File(req.filePath() + getFileName(req.url().toURI().toString()));
		...
		static Response execute(Connection.Request req, Response previousResponse) throws IOException {
            ...
            if (req.data().size() > 0 && (!methodHasBody || hasRequestBody))
                serialiseRequestUrl(req);//url
            else if (methodHasBody)
                mimeBoundary = setOutputContentType(req);

            
           HttpURLConnection conn = createConnection(req);//create connection
            Response res;
            try {
                conn.connect();
                if (conn.getDoOutput())
                ...
                if (conn.getContentLength() != 0 && req.method() != HEAD) { // -1 means unknown, chunked. sun throws an IO exception on 500 response with no content when trying to read body
                    ...
                    try {
						...
						InputStreamCacher isc = new InputStreamCacher(is);//缓存流
						inputStreamToFile(req, isc);//写到文件
                        bodyStream = conn.getErrorStream() != null ? conn.getErrorStream() : isc.getInputStream();
		...
		private static LinkedHashMap<String, List<String>> createHeaderMap(HttpURLConnection conn) {
			// the default sun impl of conn.getHeaderFields() returns header
			// values out of order
			...
			for (Map.Entry<String, List<String>> entry : resHeaders.entrySet()) {
				...
				if (name.equalsIgnoreCase("Set-Cookie")) {
					for (String value : values) {
						...
						// ignores path, date, domain, validateTLSCertificates
						// et al. req'd?
