...
public class ClassScanner {
	...
	private static Set<String> excludeJarFiles = Sets.newHashSet("charsets.jar", "deploy.jar", "cldrdata.jar", "dnsns.jar", "jaccess.jar", "jfxrt.jar",
			"localedata.jar", "nashorn.jar", "sunec.jar", "sunjce_provider.jar", "sunpkcs11.jar", "zipfs.jar", "javaws.jar", "jce.jar", "jfr.jar", "jfxswt.jar",
			"jsse.jar", "management-agent.jar", "plugin.jar", "resources.jar", "rt.jar", "ant-javafx.jar", "dt.jar", "javafx-mx.jar", "jconsole.jar",
			"packager.jar", "sa-jdi.jar", "tools.jar");
	...
	public static <T> void scan(File f, ClassVisitor<T> classVisitor, Collection<String> basePackages) {
		...
		if (f.isDirectory()) {
			...
			for (File file : classFileList) {
				...
				int end = file.toString().length() - 6; // 6 ==
														// ".class".length();
	...
	private static <T> void visitClass(String className, Collection<String> basePackages, ClassVisitor<T> classVisitor) {
