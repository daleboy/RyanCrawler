...
public class Elements extends ArrayList<Element> {
	 ...
	 * @return elements
	 */
	public Elements absLinks() {
		Elements es = new Elements();
		for (Element e : this.select("a")) {
			Element element = e.absLink();
	...
	/***
	 * Get an absolute link to all a tags
	 * 
	 * @return elements
	 */
	public Elements links() {
		return this.getDocument().xpath("//*/@href");
	}
	...
	public Elements regexExtraction(String paramString, int group) {
		...
		Elements es = new Elements();
		for (Element e : this) {
			Element element = e.regexExtraction(paramString, group);
			if (element == null || element.childNodeSize() <= 0) {
				continue;
			}
			es.add(element);
	 ...
	 * @return
	 */
	public Elements regexMatcher(String paramString) {
		Validate.notNull(paramString);

		Elements es = new Elements();
		for (Element e : this) {
			Element element = e.regexMatcher(paramString);
			if (element == null || element.childNodeSize() <= 0) {
				continue;
			}
			es.add(element);
	 ...
	 *            {@code abs:} prefix to the key to get absolute URLs from
	 *            relative URLs, e.g.:
